...
City life adds a handful of plain-JSON fields next to `s.raid`, each a phase or a timer: `s.moreel`, `s.feest`, `s.handel`, `s.opdracht`, `s.gebeurtenis`, plus `s.onderzoek`, `s.kaartmaat` and `s.moeilijkheid`. `save.js` fills them in for older saves.
