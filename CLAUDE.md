...
**The visual polish lives in `src/render/pixi-renderer.js`, on the stage and never in `Game.state`.** After the scene is composited, a handful of render-only passes run every frame, all read off `sfeer.licht(s)`: a single global **colour grade** (`werkKleurGradeBij`, a `ColorMatrixFilter` on `app.stage`) that binds every palette into one image — fuller and warmer by day, cooler and flatter at night — and is deliberately on the *stage*, not the world container, so it costs one screen-sized pass instead of a world-sized render; **atmospheric depth** (`tekenDiepte`), a blue haze that strengthens toward the top of the iso screen so far tiles recede; **wind** (`zwaaiWind`), where each tree crown is its own Graphics pivoted at its foot so a small rotation reads as a sway and flag cloth waves via a skew, one slow gust oscillator plus the rain intensity moving them together; **build juice** (`vierAfbouw` / `tickPlofjes`), a squash-and-settle plus a dust puff when a building finishes — the scale runs about the footprint centre by cancelling pivot against position, because the volume Graphics draws at absolute iso coords; **work effects** (`noteerEffectpunt` / `tickWerkeffect`): sparks at the smith, steam at the brewery/bakery, a splash at the fishery, in the same particle stream as the chimney smoke; and **water** — a pulsing coastal foam line and a soft, wavering reflection of waterside buildings (`tekenSpiegeling`, roof colour by day, warm glow at night). The ground itself gets a two-speckle **grain** baked into the static terrain Graphics (`bouwTerrein`), so the flat fills are broken with no per-frame cost.

**Icons are one set, drawn in both worlds.** `js/render/iconen.js` (`Game.render.iconen`) is a library of transform-free 24×24 SVGs that render identically in the DOM (innerHTML) and on the canvas (Pixi's `Graphics.svg`), replacing the OS-emoji on the surfaces that read as the game's own face: the HUD resource row, the building **badges** (`badgeVoorGebouw` = the icon of what the building *produces*, else a category glyph), the yield floaters and the build-menu cost chips. Everything falls back to the old emoji when an icon is missing. The boundary is deliberate: the tijdperk-eisenlijst and the tooltips still show emoji, because those are baked into config/core display strings (`ages.js` builds `r.tekst`), and iconising them would couple the simulation to the render layer — a separate, larger change.

**The first screen is a set piece.** `index.html` carries a CSS/SVG title backdrop shown only on the welcome overlay (`#overlay.titel`, toggled by `overlay.welkom`): a dawn sky, a breathing sun, drifting clouds and trekking birds, and a medieval skyline with a turning mill and a waving flag — pure CSS/SVG, independent of `Game.state`, and switched off under `prefers-reduced-motion`. Headings use a display font (the `--kop` custom property, Cinzel) loaded from Google Fonts with a graceful serif fallback.
