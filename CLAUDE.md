...
`js/render/lagen.js` + `js/ui/lagen.js` are the eye for all of this: four map overlays (voorzieningen, aantrekkelijkheid, verdediging, grondstoffen), toggled with **L** or the bar at the bottom of the stage. Treat them as part of the feature, not decoration — a locality rule the player cannot see is a locality rule they will experience as randomness.
